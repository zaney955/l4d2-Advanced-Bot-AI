...
function ChatTriggers::botban( player, args, text ) {
	BotAI.displayOptionMenuBannedWeapons( player, args, text );
}
...
function VSLib::EasyLogic::OnTakeDamage::BotAITakeDamage(damageTable) {
	...
	if(overpoweredBoost && victim != null && BotAI.IsPlayerEntityValid(attacker) && IsPlayerABot(attacker) && attacker.IsSurvivor()) {
